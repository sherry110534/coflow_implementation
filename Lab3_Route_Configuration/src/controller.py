...
class SimpleController1(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if msg.datapath.id == 1:
            # For h1-h2 flow: h1 -> s1 -> s3 (green)
            match = parser.OFPMatch(
                in_port=1,
                eth_type=0x0800,
                ipv4_src="10.0.0.1", # h1 
                ipv4_dst="10.0.0.2", # h1
                ip_proto=17,
                udp_dst=5566)
            ...
            self.add_flow(
                ...
                match=match,
                actions=actions)
            # For h2-h1 flow: s3 -> s2 -> s1 -> h1 (orange)
            match = parser.OFPMatch(
                in_port=3,
                eth_type=0x0800,
                ipv4_src="10.0.0.2", # h2
                ipv4_dst="10.0.0.1", # h1
                ip_proto=17,
                udp_dst=5566)
        ...
        if msg.datapath.id == 3:
            # For h2-h1 flow: h2 -> s3 -> s2 -> s1 (orange)
            match = parser.OFPMatch(
                in_port=1,
                eth_type=0x0800,
                ipv4_src="10.0.0.2", # h2
